/**
 * Shared SVG building blocks for the ColorCloset "Noir" CC Mark, so the launcher/adaptive
 * icons (make-icons.mjs) and the Play-store listing assets (make-store-assets.mjs) draw
 * from ONE source of truth. Keep src/components/Logo.tsx in visual sync with `mark` here.
 * Authored in a 240×240 layout.
 */
export const GOLD = '#E6C074';
export const VB = 240;
export const n = (x) => Number(x).toFixed(2);
...
// foot) with a faint rim highlight.
...
// The recognisable mark: rail + three garments + hooks (no tile).
export const mark = `
...
// Background ambiance behind the mark (door keyline, centre seam, glow).
export const keyline = `<rect x="36" y="30" width="168" height="180" rx="24" fill="none" stroke="${GOLD}" stroke-opacity="0.13" stroke-width="1.5"/>`;
export const seam = `<rect x="119.5" y="44" width="1" height="152" fill="url(#seam)"/>`;
export const glow = `<circle cx="120" cy="125" r="77" fill="url(#glow)"/>`;
export const ambiance = `${keyline}${seam}${glow}`;

export const tile = `<rect width="${VB}" height="${VB}" fill="url(#bg)"/>`;
export const sheen = `<rect width="${VB}" height="${VB}" fill="url(#sheen)"/>`;
// Centre the mark (optical centre ~120,117) and scale about the canvas centre.
export const markScaled = (k) => `<g transform="translate(120 120) scale(${n(k)}) translate(-120 -117)">${mark}</g>`;

export const svg = (inner) => `<svg xmlns="http://www.w3.org/2000/svg" width="${VB}" height="${VB}" viewBox="0 0 ${VB} ${VB}">${defs}${inner}</svg>`;

// Monochrome themed-icon silhouette (white rail + garments + hooks).
